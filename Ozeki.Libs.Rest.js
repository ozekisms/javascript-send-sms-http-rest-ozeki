...
export class MessageApi {
    ...
    async Send (messages) {
        try {
            ...
            return await this.DoRequestPost(this.createUriToSendMessage(this._configuration.ApiUrl), authHeader, requestBody);
        } catch (error) {
            return `Error: ${error}`;
        }
    }

    async Delete (folder, messages) {
        try {
    ...
    async DownloadIncoming () {
        var authHeader = this.createAuthHeader(this._configuration.Username, this._configuration.Password);
        return await this.DoRequestGet(this.createUriToReceiveMessage(this._configuration.ApiUrl, Folder.Inbox), authHeader);
    }
    ...
    async DoRequestPost (url, authHeader, requestBody, messages, method) {
        const promise = new Promise((resolve, reject)=>{
            ...
            xhr.onreadystatechange = function() {
                if (this.readyState === XMLHttpRequest.DONE && this.status === 200) {
                    if (messages) {
                        resolve( new MessageApi().getMessageResponseManipulate(xhr.response, messages, method));
                    } else {
                        resolve( new MessageApi().getMessageResponse(xhr.response));
                    }
                }
            } 
        ...
        return await promise.then(result => {return result}); 
    }
    ...
    async DoRequestGet (url, authHeader) {
        const promise = new Promise((resolve, reject)=>{
            ...
            xhr.onreadystatechange = function() {
                if (this.readyState === XMLHttpRequest.DONE && this.status === 200) {
                    resolve(new MessageApi().getResponseReceive(xhr.response));
                }
            } 
        ...
        var result = await promise.then(response => { return response });

        if (result.Messages.length > 0) {
            await this.Delete(Folder.Inbox, result.Messages);
        }
    ...
    createUriToReceiveMessage (url, folder) {
        var baseurl = url.split("?")[0];
        return `${baseurl}?action=receivemsg&folder=${folder}`;
    }

    getMessageResponse = function (response) {
        try {
    ...
    getMessageResponseManipulate = function (response, messages, method) {
        try {
    ...
    getResponseReceive = function (response) {
        try {
            ...
            return new MessageReceiveResult(folder, limit, msgs, msgs.length);
        } catch (error) {
            console.log(`Error: ${error}`);
        }
    };
...
export class MessageDeleteResult {
...
export class MessageMarkResult {
...
export class MessageReceiveResult {
    constructor(folder, limit, messages, count) {
        this.Folder = folder,
        this.Limit = limit;
